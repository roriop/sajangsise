...
- Cloudflare Pages 배포 후 KAMIS Open API 신청 검증용 라이브 사이트 운영

## Phase 1 이후

- KAMIS Open API 연동 (`src/lib/data/kamis.ts`에 API 명세 문서화됨)
- 데이터 저장: JSONL 파일 또는 SQLite (외부 매니지드 DB X)
- GH Actions 매일 18:00 KST cron → git commit → Cloudflare Pages 자동 재배포
...
Hobby/상업 구분 없이 무료 플랜에서 광고 포함 상업적 이용이 허용되어 월 운영비 ₩0 유지.

1. GitHub repo 생성 → `git remote add origin <url>` → `git push -u origin main`
2. Cloudflare 대시보드 → Workers & Pages → Create → Pages → Connect to Git → repo 선택
3. 빌드 설정: Framework preset `Astro`, Build command `npm run build`, Output directory `dist`
4. 환경변수: `USE_KAMIS_MOCK=true` (Phase 0), 나머지는 Phase 1 진입 시
5. 도메인 연결: Pages 프로젝트 → Custom domains → 도메인 추가 (DNS는 Cloudflare에 두면 자동)
